...
public class NewPost2 extends Base {
	...
	public void homePageLoginValidation(UserInfo[] userInfo) throws IOException, InterruptedException {
		...
		/*
		 * hppo.emailMethod().sendKeys(email); hppo.password().sendKeys(password);
		 * hppo.submitMethod().click();
		 * 
		 * LandingPagePO lppo = new LandingPagePO(driver);
		 * 
		 * Assert.assertEquals(lppo.usernameMethod(), username); Thread.sleep(1000);
		 */

		/*
		 * lppo.newPostMethod().click(); Thread.sleep(1000);
		 * 
		 * NewPostPO nppo = new NewPostPO(driver);
		 * 
		 * nppo.articleTitleMethod().sendKeys(articleA);
		 * nppo.whatAboutMethod().sendKeys(articleB);
		 * nppo.writeArticle().sendKeys(articleC); nppo.tagsMethod().sendKeys(articleD);
		 * nppo.publishArticleMethod().click(); Thread.sleep(1000);
		 * System.out.println(numberArticles); Assert.assertTrue(nppo.commentMethod());
		 * Thread.sleep(1000);
		 */

		/*
		 * int i=0; if(i<numberArticles) { do { lppo.newPostMethod().click();
		 * Thread.sleep(1000);
		 * 
		 * NewPostPO nppo = new NewPostPO(driver);
		 * 
		 ...
		 * nppo.writeArticle().sendKeys(articleC); nppo.tagsMethod().sendKeys(articleD);
		 * nppo.publishArticleMethod().click(); Thread.sleep(1000);
		 * Assert.assertTrue(nppo.commentMethod()); Thread.sleep(1000); i++; }while
		 * (i<numberArticles); }
		 */

		/*
		 * for(Article article: articles) { lppo.newPostMethod().click();
		 * Thread.sleep(1000);
		 * 
		 * NewPostPO nppo = new NewPostPO(driver);
		 * 
		 ...
		 * nppo.tagsMethod().sendKeys(article.tag); nppo.publishArticleMethod().click();
		 * Thread.sleep(1000); Assert.assertTrue(nppo.commentMethod());
		 * Thread.sleep(1000); }
		 */
